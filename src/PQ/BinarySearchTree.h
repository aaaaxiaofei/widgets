...
*****************************************************/
    ...
    void displayLinks(BinaryNode *t, ostream & out) const
    {
        if( t != nullptr )
        {
            out << "address:" << t << ",value:" << t->element << ", left address:" << t->left << ",right address:" << t->right << endl;
            displayLinks( t->left, out );
            displayLinks( t->right, out );
        }
    }
    ...
    int depth(BinaryNode *t, const Comparable & x) const
    {
        if (t == nullptr) {
            throw UnderflowException{ };
        }
        if (t->element == x)
            ...
            return 1 + depth(t->right, x);
        else
            return 1 + depth(t->left, x);
    }
};
